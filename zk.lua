...
function zk.notifyInfo(title, text)
  pcall(function()
    hs.notify.new({ title = tostring(title or "Info"), informativeText = tostring(text or ""), autoWithdraw = true }):send()
  end)
end
...
local function loadVaultConfig()
  ...
  if not s then
    pcall(function()
      hs.notify.new({ title = "Zettelkasten Config Missing", informativeText = "Create " .. cfgPath .. " to configure vaultName and vaultPath.", autoWithdraw = true }):send()
    end)
  ...
  local path = (type(cfg.vaultPath) == "string" and cfg.vaultPath ~= "") and expandPath(cfg.vaultPath) or nil
  ...
  if type(shots) ~= "string" or shots == "" then shots = "Screenshots" end
  local filesRel = cfg.filesFolderRel
  if type(filesRel) ~= "string" or filesRel == "" then filesRel = nil end
  local excludes = nil
  if type(cfg.excludeFoldersRel) == "table" then
    ...
    for _, v in ipairs(cfg.excludeFoldersRel) do
      if type(v) == "string" then
        local s = v:gsub("/*$", "")
        if s ~= "" and not seen[s] then table.insert(excludes, s); seen[s] = true end
      end
    end
    if #excludes == 0 then excludes = nil end
  end
  if not excludes then excludes = { "04 - Archives" } end
  local inboxRel = (type(cfg.inboxFolderRel) == "string" and cfg.inboxFolderRel ~= "") and cfg.inboxFolderRel or nil
  local inboxPrefix = (type(cfg.inboxFilePrefix) == "string" and cfg.inboxFilePrefix ~= "") and cfg.inboxFilePrefix or nil
  return {
    vaultName = name,
    vaultPath = path,
...
local function requireVaultConfig()
  if zk.config.vaultName and zk.config.vaultPath then return true end
...
local function openObsidianPath(relativePath)
  if not requireVaultConfig() then return end
...
local function sourceContextBlock(opts)
  local appName
  local url, title
  if type(opts) == "table" then appName = opts.from; url = opts.url; title = opts.title end
  if not appName then
...
local function ensureFleetingFile(relative)
  if not requireVaultConfig() then return false end
...
local function getRG()
  if _rgPathCache ~= nil then return _rgPathCache end
  _rgPathCache = findRG()
  return _rgPathCache
end
...
local function listMarkdownFilesRG(root)
  ...
  local cmd = table.concat({
    "cd", sq(root), "&&", sq(rg), "--hidden", "--no-messages", "--files",
    "-g", "'!**/.obsidian/**'", "-g", "'**/*.md'",
  }, " ")
  local out = hs.execute(cmd, true) or ""
  local files = {}
  for line in out:gmatch("[^\r\n]+") do if line ~= "" then table.insert(files, line) end end
  return files
end
...
local function listAllNotes()
  if not (zk.config.vaultPath and #zk.config.vaultPath > 0) then return {} end
  return listMarkdownFilesRG(zk.config.vaultPath) or listMarkdownFilesFallback(zk.config.vaultPath)
end

local function filterExcluded(files, excludes)
  if not files or #files == 0 then return files end
  if not excludes or #excludes == 0 then return files end
  local filtered = {}
  for _, p in ipairs(files) do
    ...
    if not skip then table.insert(filtered, p) end
  end
  return filtered
end
...
function zk.openFleeting()
  ...
  if not ensureFleetingFile(relative) then return end
...
function zk.captureText(params)
  ...
  if text == "" then zk.alertError("capture: empty text"); return end
  local relative = zk.todayFleetingRelative()
  if not ensureFleetingFile(relative) then return end
  local full = pathJoin(zk.config.vaultPath, relative)
  ...
  if appendFile(full, entry) then openObsidianPath(relative) end
end
...
function zk.captureScreenshot()
  ...
  if not bin then zk.alertError("screencapture not found"); return end
  ...
  local task = hs.task.new(bin, function(exitCode, _, stdErr)
    if exitCode ~= 0 then zk.alertError("Screenshot failed: " .. tostring(stdErr or exitCode)); return end
    if not hs.fs.attributes(outFull) then zk.alertError("No file saved"); return end
    local body = string.format("![[%s]]", relative)
    local relF = zk.todayFleetingRelative()
    if not ensureFleetingFile(relF) then return end
